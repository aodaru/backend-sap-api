...
from services.sap_errors import SapBusinessError, SapConnectionError, SapExecutionTimeoutError, SapIntegrationError, safe_exception
...
class SapTransactionExecutor:
    ...
    async def execute(
        ...
        settings = get_settings()
        if getattr(settings, "sap_integration_enabled", False) and transaction.upper() == "VK12":
